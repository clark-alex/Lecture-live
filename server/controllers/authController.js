  ...
  registerUser: (req, res) => {
    ...
    db.verify_user([username]).then(user => {
      if (user.length !== 0) {
        ...
        db.register_user([username, hash, email, status]).then(userInfo => {
          req.session.user.session_id = userInfo[0].user_id;
          req.session.user.user_id = userInfo[0].user_id;
          req.session.user.username = userInfo[0].username;
